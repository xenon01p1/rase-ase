...
$(function(){
    ...
    $('#search').on('change', function(){
        $criteria = $(this).val();
        $url = '../../model/admin/model_courier.php?action=search&by=' + $criteria;
        $('#rows').empty();
        show_data($url, 1);
    });
    ...
    $('#search_type').on('input', function(){
        $value = $(this).val();
        $url = '../../model/admin/model_courier.php?action=search_type&value=' + $value ;
        $('#rows').empty();
        show_data($url, 1);
    });
 ...
 function show_data(arg_url='', type=0){
    ...
    if(type == 0){
        url = "../../model/admin/model_courier.php?action=table_data";
    }else{
        url = arg_url;
    }
...
// used in base action function
 function delete_data(id){
    if(confirm("Are you sure?")){
        $.ajax({
           url : "../../model/admin/model_courier.php?action=delete&id="+id,
           type : "GET",
           success : function(data){
                console.log(data);
                console.log("success!");
 ...
 function edit_data(kurir_id){
    $('#content').load('courier_add.php?edit=1&kurir_id='+kurir_id);	
 }

//  function detail_data(courier_id){
//     $('#content').load('view/admin/courier_add.php?courier_id='+ courier_id + '&detail=1');	
//  }
